...
# ── IDI Tenders scraper (listing page, direct PDF download) ───────────────

def _run_idi_scrape(site_key: str, keywords: list, log_cb,
                    result_cb=None, team_id: str = "cnk") -> list[Path] | None:
    from agents.idi_scraper_agent import IDIScraperAgent
    from agents.file_reader import read_file
    from agents.excel_writer import write_level1_report
    from date_utils import get_max_age_hours
    max_age_hours = get_max_age_hours(team_id)

    agent      = IDIScraperAgent()
    summarizer = SummarizerAgent()
    timestamp  = now_ist_naive().strftime("%Y%m%d_%H%M%S")
    run_dir    = DOWNLOADS_DIR / site_key / timestamp
    tender_dirs: list[Path] = []

    log_cb("🚀 Starting IDI Tenders Scraper...")
...
# ── LIC Tenders scraper (table listing + detail pages) ───────────────────

def _run_lic_scrape(site_key: str, keywords: list, log_cb,
                    result_cb=None, team_id: str = "tmi") -> list[Path] | None:
    from agents.lic_scraper_agent import LICScraperAgent
    from agents.file_reader import read_file
    from agents.excel_writer import write_level1_report
    from date_utils import get_max_age_hours
    max_age_hours = get_max_age_hours(team_id)

    agent      = LICScraperAgent()
    summarizer = SummarizerAgent()
    timestamp  = now_ist_naive().strftime("%Y%m%d_%H%M%S")
    run_dir    = DOWNLOADS_DIR / site_key / timestamp
    tender_dirs: list[Path] = []

    log_cb("🚀 Starting LIC Tenders Scraper...")
...
# ── ISA (International Solar Alliance) procurement scraper ───────────────

def _run_isa_scrape(site_key: str, keywords: list, log_cb,
                    result_cb=None, team_id: str = "cnk") -> list[Path] | None:
    from agents.isa_scraper_agent import ISAScraperAgent
    from agents.file_reader import read_file
    from agents.excel_writer import write_level1_report

    agent      = ISAScraperAgent()
    summarizer = SummarizerAgent()
    timestamp  = now_ist_naive().strftime("%Y%m%d_%H%M%S")
    run_dir    = DOWNLOADS_DIR / site_key / timestamp
    tender_dirs: list[Path] = []

    log_cb("🚀 Starting ISA Procurement Scraper...")
    ...
    if not tender_dirs:
        log_cb("ℹ️ Run finished — nothing new to save. Everything listed was either "
               "closed for bids, already collected in an earlier run, "
               "or didn't match your keywords.")
        return None
...
# ── Mercy Corps tenders scraper ──────────────────────────────────────────

def _run_mercycorps_scrape(site_key: str, keywords: list, log_cb,
                           result_cb=None, team_id: str = "cnk") -> list[Path] | None:
    from agents.mercycorps_scraper_agent import MercyCorpsScraperAgent
    from agents.file_reader import read_file
    from agents.excel_writer import write_level1_report

    agent      = MercyCorpsScraperAgent()
    summarizer = SummarizerAgent()
    timestamp  = now_ist_naive().strftime("%Y%m%d_%H%M%S")
    run_dir    = DOWNLOADS_DIR / site_key / timestamp
    tender_dirs: list[Path] = []

    log_cb("🚀 Starting Mercy Corps Tenders Scraper...")
            ...
            try:
                ...
                if site_key == "ungm":
                    ...
                    if result:
                        ...
                        base = DOWNLOADS_DIR / site_key
                        zip_path = _make_run_zip(result, base, f"{site_key}_{ts}")
                elif _scraper_type == "idi":
                    result = _run_idi_scrape(site_key, keywords, log_cb, result_cb, team_id=team_id)
                    if result:
                        log_cb("📦 Packaging all results into ZIP...")
                        base = DOWNLOADS_DIR / site_key
                        zip_path = _make_run_zip(result, base, f"{site_key}_{ts}")
                elif _scraper_type == "lic":
                    result = _run_lic_scrape(site_key, keywords, log_cb, result_cb, team_id=team_id)
                    if result:
                        log_cb("📦 Packaging all results into ZIP...")
                        base = DOWNLOADS_DIR / site_key
                        zip_path = _make_run_zip(result, base, f"{site_key}_{ts}")
                elif _scraper_type == "isa":
                    result = _run_isa_scrape(site_key, keywords, log_cb, result_cb, team_id=team_id)
                    if result:
                        log_cb("📦 Packaging all results into ZIP...")
                        base = DOWNLOADS_DIR / site_key
                        zip_path = _make_run_zip(result, base, f"{site_key}_{ts}")
                elif _scraper_type == "mercycorps":
                    result = _run_mercycorps_scrape(site_key, keywords, log_cb, result_cb, team_id=team_id)
                    if result:
                        log_cb("📦 Packaging all results into ZIP...")
                        base = DOWNLOADS_DIR / site_key
                        zip_path = _make_run_zip(result, base, f"{site_key}_{ts}")
