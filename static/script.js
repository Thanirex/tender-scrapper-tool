    ...
    authFetch(`/config?team_id=${currentTeamId}`)
        ...
        .then(data => {
            ...
            siteSelect.innerHTML = data.sites.map(s => {
                const label = s === 'ungm' ? 'UNGM (UN Global Marketplace)' : s === 'au' ? 'African Union Bids' : s === 'acbf' ? 'ACBF Procurement & Consultancies' : s === 'trademarkafrica' ? 'TradeMark Africa Procurement' : s === 'isa' ? 'ISA (International Solar Alliance) Procurement' : s === 'mercycorps' ? 'Mercy Corps Tenders (RFPs)' : s.toUpperCase();
